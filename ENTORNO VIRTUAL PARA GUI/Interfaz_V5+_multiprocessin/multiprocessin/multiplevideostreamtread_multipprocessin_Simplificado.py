...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.processor_input_queue = self.manager.Queue()
        self.processor_output_queue = self.manager.Queue()
    ...
    def list_python_processes(self):
        for process in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                if 'python' in process.info['name'].lower():
                    print(f"PID: {process.info['pid']}, Name: {process.info['name']}, Command: {' '.join(process.info['cmdline'])}")
