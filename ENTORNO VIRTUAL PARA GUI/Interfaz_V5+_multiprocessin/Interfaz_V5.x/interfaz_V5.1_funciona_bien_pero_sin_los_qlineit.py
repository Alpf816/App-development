...
def obtener_info_cpu():
    try:
        ...
        # Obtiene el conjunto de afinidad de CPU del proceso
        afinidad_cpu = proceso.cpu_affinity()
        # Obtiene el número de núcleo actual
        nucelo_actual = psutil.cpu_count(logical=False) - 1
        return afinidad_cpu, nucelo_actual
    except Exception as e:
        print(f"No se pudo obtener la información de la CPU: {e}")
        return None, None
# Obtiene la información de la CPU
afinidad_cpu, nucelo_actual = obtener_info_cpu()
# Imprime la información obtenida
print(f"Conjunto de afinidad de CPU: {afinidad_cpu}")
print(f"Núcleo actual: {nucelo_actual}")
...
class VideoWidget(QWidget):
    def __init__(self ):
        ...
        try:
            ...
            self.edit_text9 = QLineEdit(self)
            self.edit_text10 = QLineEdit(self)
            self.edit_text11 = QLineEdit(self)
            self.edit_text12 = QLineEdit(self)
            self.edit_text13 = QLineEdit(self)
            self.edit_text14 = QLineEdit(self)
            self.edit_text15 = QLineEdit(self)
            self.edit_text16 = QLineEdit(self)
            print("punto de interrupcion 1")
            ...
            self.edit_text9.setValidator(int_validator)
            self.edit_text10.setValidator(int_validator)
            self.edit_text11.setValidator(int_validator)
            self.edit_text12.setValidator(int_validator)
            self.edit_text13.setValidator(int_validator)
            self.edit_text14.setValidator(int_validator)
            self.edit_text15.setValidator(int_validator)
            self.edit_text16.setValidator(int_validator)
            print("punto de interrupcion 2")
            self.edit_text1.setPlaceholderText("X ROI 1")
            self.edit_text2.setPlaceholderText("Y ROI 1")
            self.edit_text3.setPlaceholderText("Ancho ROI 1")
            self.edit_text4.setPlaceholderText("Alto ROI 1")
            self.edit_text5.setPlaceholderText("X ROI 2")
            self.edit_text6.setPlaceholderText("Y ROI 2")
            self.edit_text7.setPlaceholderText("Ancho ROI 2")
            self.edit_text8.setPlaceholderText("Alto ROI 2")
            self.edit_text9.setPlaceholderText("X ROI 3")
            self.edit_text10.setPlaceholderText("Y ROI 3")
            self.edit_text11.setPlaceholderText("Ancho ROI 3")
            self.edit_text12.setPlaceholderText("Alto ROI 3")
            self.edit_text13.setPlaceholderText("X ROI 4")
            self.edit_text14.setPlaceholderText("Y ROI 4")
            self.edit_text15.setPlaceholderText("Ancho ROI 4")
            self.edit_text16.setPlaceholderText("Alto ROI 4")
            print("punto de interrupcion 3")
            # Add labels for QLineEdit widgets
            label1 = QLabel("Origen ROI 1:", self)
            label2 = QLabel("WH_ROI_1:", self)
            label3 = QLabel("Origen ROI 2:", self)
            label4 = QLabel("WH_ROI_2:", self)
            label5 = QLabel("Origen ROI 3:", self)
            label6 = QLabel("WH_ROI_3:", self)
            label7 = QLabel("Origen ROI 4:", self)
            label8 = QLabel("WH_ROI_4:", self)
            ...
            for edit_widget in [self.edit_text1, self.edit_text2, self.edit_text3, self.edit_text4,
                                ...
                                self.edit_text9, self.edit_text10, self.edit_text11, self.edit_text12,
                                self.edit_text13, self.edit_text14, self.edit_text15, self.edit_text16]:
                edit_widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            # Set up the layout
            print("punto de interrupcion 4")
            layout.addWidget(label1, 0,0,2,1)
            layout.addWidget(self.edit_text1, 0, 1)
            layout.addWidget(self.edit_text2, 1, 1)
            layout.addWidget(label2, 2,0,2,1)
            layout.addWidget(self.edit_text3, 2, 1)
            layout.addWidget(self.edit_text4, 3, 1)
            layout.addWidget(label3, 0, 2,2,1)
            layout.addWidget(self.edit_text5, 0, 3)
            layout.addWidget(self.edit_text6, 1, 3)
            layout.addWidget(label4, 2, 2,2,1)
            layout.addWidget(self.edit_text7, 2, 3)
            layout.addWidget(self.edit_text8, 3, 3)
            layout.addWidget(label5, 0, 4,2,1)
            layout.addWidget(self.edit_text9, 0, 5)
            layout.addWidget(self.edit_text10, 1, 5)
            layout.addWidget(label6, 2, 4,2,1)
            layout.addWidget(self.edit_text11, 2, 5)
            layout.addWidget(self.edit_text12, 3, 5)
            layout.addWidget(label7, 0, 6,2,1)
            layout.addWidget(self.edit_text13, 0, 7)
            layout.addWidget(self.edit_text14, 1, 7)
            layout.addWidget(label8, 2, 6,2,1)
            layout.addWidget(self.edit_text15, 2, 7)
            layout.addWidget(self.edit_text16, 3, 7)

            layout.addWidget(self.label_video, 5,0,1,8)  # Adjust the row span for the video label
            print("punto de interrupcion 5")
            ...
            self.edit_text9.textChanged.connect(self.on_text_changed)
            self.edit_text10.textChanged.connect(self.on_text_changed)
            self.edit_text11.textChanged.connect(self.on_text_changed)
            self.edit_text12.textChanged.connect(self.on_text_changed)
            self.edit_text13.textChanged.connect(self.on_text_changed)
            self.edit_text14.textChanged.connect(self.on_text_changed)
            self.edit_text15.textChanged.connect(self.on_text_changed)
            self.edit_text16.textChanged.connect(self.on_text_changed)
            


            print("punto de interrupcion 6")
        except Exception as e:
            print(f"Error: {e}")
            
    def on_text_changed(self):
        print("punto de interrupcion 7")
        t1 = int(self.edit_text1.text()) if self.edit_text1.text() else 0
        t2 = int(self.edit_text2.text()) if self.edit_text2.text() else 0
        t3 = int(self.edit_text3.text()) if self.edit_text3.text() else 0
        t4 = int(self.edit_text4.text()) if self.edit_text4.text() else 0
        t5 = int(self.edit_text5.text()) if self.edit_text5.text() else 0
        t6 = int(self.edit_text6.text()) if self.edit_text6.text() else 0
        t7 = int(self.edit_text7.text()) if self.edit_text7.text() else 0
        t8 = int(self.edit_text8.text()) if self.edit_text8.text() else 0
        t9= int(self.edit_text9.text()) if self.edit_text9.text() else 0
        t10 = int(self.edit_text10.text()) if self.edit_text10.text() else 0
        t11 = int(self.edit_text11.text()) if self.edit_text11.text() else 0
        t12 = int(self.edit_text12.text()) if self.edit_text12.text() else 0
        t13 = int(self.edit_text13.text()) if self.edit_text13.text() else 0
        t14 = int(self.edit_text14.text()) if self.edit_text14.text() else 0
        t15 = int(self.edit_text15.text()) if self.edit_text15.text() else 0
        t16 = int(self.edit_text16.text()) if self.edit_text16.text() else 0
        print("punto de interrupcion 8")
...
#generacion de UI Principal-----------------------------------------
    def inicializarUI(self):
        ...
        self.setWindowTitle("Interfaz_v5.0 implementacion de process") 
