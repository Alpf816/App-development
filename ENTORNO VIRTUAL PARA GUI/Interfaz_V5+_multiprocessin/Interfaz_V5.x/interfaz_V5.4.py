            ...
            if frame is not None and frame.size != 0:
                if not self.paused:
                    if self.current_preset == "Original":
                        ...
                        #print("se Inicio Seleccion de colores")
                        self.hilo_video.Iniciar_captura_frame_ROI1()
                        
                        #global width_video_capture, height_video_capture
                        #if self.O_ROI_C1 and self.WH_ROI_C1 and self.input_queue_ROI1 and self.output_queue_ROI1 and self.WH_ROI_C1[0] and self.WH_ROI_C1[1] and self.TonMin1 and self.TonMax1 and self.PurMin1 and self.PurMax1 and self.LumMin1 and self.LumMax1 and self.KerX1 and self.KerY1:
                        if True:
                            try:
                                if not self.process_1.is_alive():
                                    self.process_1.start()
                                print("Señal de frame conectada")
                                if self.process_1.is_alive():
                                    print("Se inicio el Process 1")
                                
                                while self.señal_run_ROI1.value:
                                    print("se esta ejecutando el bucle")
                                    ...
                                    #print("Procesando frame en el bucle while")
                                    print(f"Current value of señal_run_ROI1: {self.señal_run_ROI1.value}")
                                print("Se va a finalizar el process1")
                            except Exception as e:
                                print(f"Excepción en el hilo de ImaeProcessor (seleccion de colores): {e}")
                            finally:
                                print("Se inicio la Finalizacion de process1")
                                ...
                                print("No se pudo Iniciar el Process 1")  
...
class Procesador_por_Paralelismo(Process):
    ...
    def run(self):
        #while self.señal_run_ROI1 is True:
            try:
                while self.señal_run_ROI1.value == True:
...
#generacion de UI Principal-----------------------------------------
    def inicializarUI(self):
        ...
        self.setWindowTitle("Interfaz_v5.4 ROI implementado pero sin finalizacion correcta") 
...
#Primer dock---------------------------------------------------
    ...
    def create_dock_2(self):
        midockwidget = MiDockWidget(self.video_thread,self.dic_thread,self.input_queue_ROI1,self.input_queue_ROI2,self.input_queue_ROI3,self.input_queue_ROI4)
    ...
    def closeEvent(self, event):
        try:
            ...
            #self.video_thread.stop() crashea
            #self.worker_thread.stop()crashea
            self.dic_thread.stop()

            
            print("se cierran los treads")
        finally:
            # Explicitly delete references to objects or variables
