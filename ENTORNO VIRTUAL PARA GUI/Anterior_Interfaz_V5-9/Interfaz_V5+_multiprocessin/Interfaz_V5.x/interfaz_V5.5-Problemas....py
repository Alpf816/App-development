...
class VideoWidget(QWidget):
    ...
    def __init__(self,dic_thread,hilo_video):
        super().__init__()
...
class MiVentana(QWidget):
    def __init__(self, video_thread,dic_thread):
        ...
        self.video_widget = VideoWidget(self.dic_thread,self.hilo_video)
    ...
    def iniciar_captura(self):
        if not self.hilo_video.isRunning() and self.hilo_video is not None:
            self.hilo_video.start()
            print("start hilo_video")

    def detener_captura(self):
        self.hilo_video.detener_captura()
        self.hilo_video.quit()
        self.hilo_video.wait()
        print("videostreamtread Finalizado")
...
class MiDockWidget(QWidget):
    ...
    def start_camera(self):
        if not self.hilo_video.isRunning() and self.hilo_video is not None:
            ...
            print("start image_processor_thread")
...
class ImageProcessorThread(QThread):
    ...
    def __init__(self, get_frame_function, dic_thread,input_queue_ROI1,input_queue_ROI2,input_queue_ROI3,input_queue_ROI4,hilo_video):
        ...
        print("init imaeprocesortread")

    def run(self):
        #self.process_1 = None
        self.process_1 = Process_de_ejemplo(self.input_queue_ROI1,self.output_queue_ROI1)
            ...
            if frame is not None and frame.size != 0:
                if not self.paused:
                    if self.current_preset == "Original":
                        ...
                        if self.señal_run_ROI1:
                    
                        #if self.O_ROI_C1 is not None and self.WH_ROI_C1 is not None and self.WH_ROI_C1[0] is not None and self.WH_ROI_C1[1] is not None and self.TonMin1 is not None and self.TonMax1 is not None and self.PurMin1 is not None and self.PurMax1 is not None and self.LumMin1 is not None and self.LumMax1 is not None and self.KerX1 is not None and self.KerY1:
                            #self.process_1 = Procesador_por_Paralelismo(self.O_ROI_C1,self.WH_ROI_C1,self.input_queue_ROI1,self.output_queue_ROI1,self.WH_ROI_C1[0],self.WH_ROI_C1[1],self.TonMin1,self.TonMax1,self.PurMin1,self.PurMax1,self.LumMin1,self.LumMax1,self.KerX1,self.KerY1,self.señal_run_ROI1)
                            
                            self.señal_run_ROI1_dentro_del_bucle()
                            print("Se esta iniciando Seleccion de colores")
                            if not self.process_1.is_alive() and self.process_1 is None:
                                try:
                                    self.process_1 = Process_de_ejemplo(self.input_queue_ROI1, self.output_queue_ROI1)
                                    self.process_1.start()
                                    print("Process 1 iniciado correctamente")
                                    while self.señal_run_ROI1:
                                        frame_ROI_1_processed = self.output_queue_ROI1.get()
                                        q_image_ROI1 = QImage(frame_ROI_1_processed.data, self.WH_ROI_C1[0], self.WH_ROI_C1[1], self.WH_ROI_C1[0] * 3, QImage.Format.Format_RGB888)
                                        self.processed_frame_signal.emit(q_image_ROI1)
                                except Exception as e:
                                    print(f"Error al iniciar el Process 1: {e}")
                            else:
                                print("Process 1 ya está en ejecución")
                                self.señal_run_ROI1 = False
                                self.hilo_video.parar_captura_frame_ROI1()
                                if self.process_1 is not None and self.process_1.is_alive():
                                    print("Process 1 is still alive. Terminating...")
                                    self.process_1.terminate()
                                    self.process_1.join()
                                    print("Process 1 terminated.")
                                
                        else:
                            print("No se pudo Iniciar el Process 1,No se cumplen los requisitos")
                    ...
                    QThread.msleep(60)
    def señal_run_ROI1_dentro_del_bucle(self):
    ...
    def stop_process_1(self):
        try:
            ...
            print(f"Excepción en stop_process_1: {e}")
    ...
    def reanudar(self):
        ...
        self.señal_run_ROI1 = True

class Procesador_por_Paralelismo(multiprocessing.Process):
    def __init__(self,O_ROI_C,WH_ROI_C,input_queue_ROI,output_queue_ROI,width_video_capture, height_video_capture,TonMin,TonMax,PurMin,PurMax,LumMin,LumMax,KerX,KerY,señal_run_ROI1):
        super().__init__()
        ...
        self.señal_run_ROI1 = señal_run_ROI1
        print("Procesador_por_Paralelismo __init__ called")
    def run(self):
    
        print("se inicio el process 1") 
        while self.señal_run_ROI1:
        ...
        print("Procesador_por_Paralelismo process is exiting.")

class Process_de_ejemplo (multiprocessing.Process):
    def __init__(self,input_queue_ROI,output_queue_ROI):
        self.input_queue = input_queue_ROI
        self.output_queue = output_queue_ROI

    def run(self):
        print("se inicio el process de ejemplo") 
        while True:
            if not self.input_queue.empty():
                frame = self.input_queue.get()
                self.output_queue.put(frame)
...
class Ventana_Principal(QMainWindow):
    ...
    def closeEvent(self, event):
        try:
            ...
            #self.worker_thread.stop1()
            print("se cierran los treads")
        finally:
    ...
    def stop_threads(self):
        ...
        if self.midockwidget.image_processor_thread is not None and self.midockwidget.image_processor_thread.isRunning():
            self.midockwidget.image_processor_thread.stop_process_1()
            self.midockwidget.image_processor_thread.quit()
            self.midockwidget.image_processor_thread.stop1()
...
#generacion de UI Principal-----------------------------------------
    def inicializarUI(self):
        ...
        self.setWindowTitle("Interfaz_v5.5 ROI implementado funciona a medias") 
...
#Primer dock---------------------------------------------------
    ...
    def create_dock_modificacion_de_datos(self):
        ...
        print("start worker_thread")
        self.dic_thread.start()
        print("start dic_thread")
    ...
    def create_ventana_central(self):
        ...
        ventana_video_principal = MiVentana(self.video_thread,self.dic_thread)
