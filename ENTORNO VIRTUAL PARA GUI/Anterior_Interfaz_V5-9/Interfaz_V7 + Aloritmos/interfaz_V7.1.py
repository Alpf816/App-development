...
class VideoStreamThread(QThread):
    ...
    def __init__(self,input_queue_ROI1,input_queue_ROI2,input_queue_ROI3,input_queue_ROI4):
        ...
        self.output_queue_P_A = multiprocessing.Queue(maxsize= 1)
        self.Process_P_y_A = process_proyeccion_y_aloritmos(self.input_queue_Valores_estaticos,self.input_queue_Valores_Variables,self.output_queue_P_A)
    ...
    def run(self):
        try:
            ...
            while self.capturando:
                ...
                global cords_ROI_1,cords_ROI_2, cords_ROI_3, cords_ROI_4,cm_ancho_entre_puntos,cm_alto_entre_puntos,camera_matrix,distortion_parameters,XYZ_Calculados_P1,XYZ_Calculados_P2,XYZ_Calculados_P3,XYZ_Calculados_P4
                if cords_ROI_1 is not None and cords_ROI_2 is not None and cords_ROI_3 is not None and cords_ROI_4  is not None:
                    ...
                    patron = np.array([[1, cm_alto_entre_puntos,0],     # Punto 1: X = 0, Y = 10
                        [cm_ancho_entre_puntos, cm_alto_entre_puntos,0],  # Punto 2: X = 5, Y = 10 
                        [1, 1,0],                       # Punto 3: X = 0, Y = 0 (Origen de coordenadas)
                        [cm_ancho_entre_puntos, 1,0]],  # Ultimo punto
                        dtype=np.float32)
                    diccionario_P_A = {"patron": patron,"camera matrix": camera_matrix,"distortion_parameters": distortion_parameters,"cords_ROI_1":cords_ROI_1,"cords_ROI_2":cords_ROI_2, "cords_ROI_3":cords_ROI_3, "cords_ROI_4":cords_ROI_4}
                    self.input_queue_Valores_Variables.put(diccionario_P_A)

                    try:
                        if not self.Process_P_y_A.is_alive() and self.Process_P_y_A is not None:
                                    self.Process_P_y_A.start()
                    except Exception as e:
                        print(f"No se pudo iniciar process_proyeccion_y_aloritmos: {e}")
            ...
            self.iniciar_captura_frame_ROI3 = False
            self.iniciar_captura_frame_ROI4 = False
            self.Finalizar_process_P_A()
            video_stream.release()
            self.finalizado.emit()
            print("videostreamtread finalizado")


    def Finalizar_process_P_A(self):
        self.Process_P_y_A.finalizar_process_interno()
        #self.Process_P_y_A.terminate()
        #self.Process_P_y_A.join()
...
class process_proyeccion_y_aloritmos(multiprocessing.Process):
    def __init__(self,input_queue_Valores_estaticos,input_queue_Valores_Variables,output_queue_P_A):
        super().__init__()
        self.input_queue_Valores_estaticos = input_queue_Valores_estaticos
        self.input_queue_Valores_Variables = input_queue_Valores_Variables
        self.output_queue_P_A = output_queue_P_A
        self.cords_ROI_1 = None
        self.cords_ROI_2 = None
        self.cords_ROI_3 = None
        self.cords_ROI_4 = None
        self.process_flag_interno = True
        self.patron = None
        self.centros_ordenados = None
        self.camera_matrix = None
        self.distortion_parameters = None
        print("Init Process proyeccion y Aloritmos")

    def run(self):
        print("process P_A iniciado")
        while self.process_flag_interno:
            if not self.input_queue_Valores_Variables.empty():
                self.P_A_DIC = self.input_queue_Valores_Variables.get()
                if isinstance(self.P_A_DIC, dict):
                    self.cords_ROI_1 = self.P_A_DIC.get("cords_ROI_1")
                    self.cords_ROI_2 = self.P_A_DIC.get("cords_ROI_2")
                    self.cords_ROI_3 = self.P_A_DIC.get("cords_ROI_3")
                    self.cords_ROI_4 = self.P_A_DIC.get("cords_ROI_4")
                    self.patron = self.P_A_DIC.get("patron")
                    self.camera_matrix = self.P_A_DIC.get("camera matrix")
                    self.distortion_parameters = self.P_A_DIC.get("distortion_parameters")
                    #print(self.ROI_DIC)
                else:
                    print("Se envio el dict pero no lo contiene los datos")
            try:
                if self.patron is not None and self.cords_ROI_1 is not None and self.cords_ROI_2 is not None and self.cords_ROI_3 is not None and self.cords_ROI_4 is not None and self.camera_matrix is not None and self.distortion_parameters is not None:
                    self.centros_ordenados = np.array([self.cords_ROI_1, self.cords_ROI_3, self.cords_ROI_4,self.cords_ROI_2],dtype=np.float32)
                    ret1, rvec, tvec = cv2.solvePnP(self.patron,
                        self.centros_ordenados,
                        self.camera_matrix, # Obtenida durante la calibración
                        self.distortion_parameters)  # Obtenido durante la calibración}
                    if ret1:
                        XYZ_Calculados_P1 = self.CalcularXYZ(self.cords_ROI_1[0],self.cords_ROI_1[1], self.camera_matrix, rvec, tvec)
                        XYZ_Calculados_P3 = self.CalcularXYZ(self.cords_ROI_2[0],self.cords_ROI_2[1], self.camera_matrix, rvec, tvec)
                        XYZ_Calculados_P4 = self.CalcularXYZ(self.cords_ROI_3[0],self.cords_ROI_3[1], self.camera_matrix, rvec, tvec)
                        XYZ_Calculados_P2 = self.CalcularXYZ(self.cords_ROI_4[0],self.cords_ROI_4[1], self.camera_matrix, rvec, tvec)
                        print(XYZ_Calculados_P1,XYZ_Calculados_P2,XYZ_Calculados_P3,XYZ_Calculados_P4)
                        rectanle_points =  np.array([XYZ_Calculados_P1,XYZ_Calculados_P2,XYZ_Calculados_P3,XYZ_Calculados_P4])
                        #self.plot_rectangle_3d(rectanle_points)
                
                time.sleep(0.1)
            

            except Exception as e:
                print(f"Excepción en el process de P y A2: {e}")
                time.sleep(0.5)
            time.sleep(0.2)
        print("Se Finalizo el process P y A")
           
    def finalizar_process_interno(self):
        self.process_flag_interno = False


    def plot_rectangle_3d(self,points):
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        # Desempaquetar los puntos
        x, y, z = points[:, 0], points[:, 1], points[:, 2]
        # Conectar los puntos para formar el rectángulo
        ax.plot([x[0], x[1]], [y[0], y[1]], [z[0], z[1]], 'k-')
        ax.plot([x[1], x[2]], [y[1], y[2]], [z[1], z[2]], 'k-')
        ax.plot([x[2], x[3]], [y[2], y[3]], [z[2], z[3]], 'k-')
        ax.plot([x[3], x[0]], [y[3], y[0]], [z[3], z[0]], 'k-')
    ...
    def CalcularXYZ(self,u,v, A, rvec, tvec):
    # Generamos el vector m
        uv = np.array([[u,v,1]], dtype=float).T
...
class DICThread(QThread):
    ...
    def __init__(self):
        super().__init__()
        self.diccionario_datos = {"Tonalidad Minima S1": 5, "Tonalidad Maxima S1": 255, "Pureza Minima S1": 60,"Pureza Maxima S1": 255,"Luminosidad Minima S1":50,"Luminosidad Maxima S1":255,"Kernel X S1":1,"Kernel Y S1" :1,
                                  "Tonalidad Minima S2": 5, "Tonalidad Maxima S2": 255, "Pureza Minima S2": 60,"Pureza Maxima S2": 255,"Luminosidad Minima S2":50,"Luminosidad Maxima S2":255,"Kernel X S2":1,"Kernel Y S2" :1,
                                  "Tonalidad Minima S3": 5, "Tonalidad Maxima S3": 255, "Pureza Minima S3": 60,"Pureza Maxima S3": 255,"Luminosidad Minima S3":50,"Luminosidad Maxima S3":255,"Kernel X S3":1,"Kernel Y S3" :1,
                                  "Tonalidad Minima S4": 5, "Tonalidad Maxima S4": 255, "Pureza Minima S4": 60,"Pureza Maxima S4": 255,"Luminosidad Minima S4":50,"Luminosidad Maxima S4":255,"Kernel X S4":1,"Kernel Y S4" :1,
...
#Primer dock---------------------------------------------------
    ...
    def create_dock_modificacion_de_datos(self):
        ...
        #Ingreso de datos-----------------------------------------------------------------
        self.main_box_dock1.addWidget(QLabel('cm_ancho_entre_puntos:'), 2, 0)
        self.main_box_dock1.addWidget(QLineEdit(), 2, 1)
        self.main_box_dock1.addWidget(QLabel('cm_alto_entre_puntos:'), 3, 0)
        self.main_box_dock1.addWidget(QLineEdit(), 3, 1)
        self.main_box_dock1.addWidget(QPushButton('Modificar'), 4, 0,1,2)
        boton_modificar_distancias = self.main_box_dock1.itemAtPosition(4, 0).widget()
        boton_modificar_distancias.clicked.connect(self.actualizar_cm_entre_puntos)
        
        #Filtro RGB---------------------------------------------------------------
        text2 = QLabel("Filtro RGB")
        text2.setFixedHeight(20)
        text2.setStyleSheet("background-color: #dadada")
        self.main_box_dock1.addWidget(text2, 6, 0,1,2)
        ...
        self.main_box_dock1.addWidget(self.Widgget_contenedor_de_sliders,7,0,1,2)
        

        self.main_box_dock1.addWidget(scroll_area,9,0,1,2)
    ...
    def stop_threads(self):
        ...
        self.video_thread.Finalizar_process_P_A()
