...
class VideoStreamThread(QThread):
    ...
    def stop(self):
        ...
        self.wait()  
class matplotlib(multiprocessing.Process):
    def __init__(self):               
        super().__init__()
        self.process_flag_interno = True
        print("init de matplotlib")
    ...
    def run(self):
        if self.process_flag_interno:
            print("se inicio matplotlib")
...
class VideoProcessingThread(QThread):
    ...
    def actualizar_cordenadas(self, Valor_process,dic_roi):
        ...
        if Valor_process == 1:
            ...
            if flag_creacion_3D:
                Dictvect.append({"rvecs_1": rvecs_1})
                Dictvect.append({"tvecs_1": tvecs_1})
                Dictvect.append({"corners_reales_1": corners_reales_1})
            ...
            if flag_creacion_3D:
                Dictvect.append({"rvecs_2": rvecs_2})
                Dictvect.append({"tvecs_2": tvecs_2})
                Dictvect.append({"corners_reales_2": corners_reales_2})
            ...
            if flag_creacion_3D:
                Dictvect.append({"rvecs_3": rvecs_3})
                Dictvect.append({"tvecs_3": tvecs_3})
                Dictvect.append({"corners_reales_3": corners_reales_3})
            ...
            if flag_creacion_3D:
                Dictvect.append({"rvecs_4": rvecs_4})
                Dictvect.append({"tvecs_4": tvecs_4})
                Dictvect.append({"corners_reales_4": corners_reales_4})
...
class Ventana_Principal(QMainWindow):
    def __init__(self):
        super().__init__()
        try:
            self.matplotlib_process = matplotlib()
...
#generacion de UI Principal-----------------------------------------
    def inicializarUI(self):
        ...
        self.setWindowTitle("Interfaz_v8.0") 
...
#Primer dock---------------------------------------------------
    ...
    def create_dock_modificacion_de_datos(self):
        ...
        self.radio_modificar = QRadioButton("Actualizar\nautomáticamente")
        
        self.radio_eliminar = QRadioButton("No actualizar")
        self.radio_modificar.setChecked(True)  # Establecer la opción por defecto
        self.main_box_dock1.addWidget(self.radio_modificar, 5, 0)
        self.main_box_dock1.addWidget(self.radio_eliminar, 5, 1)
         # Botón de acción
        self.main_box_dock1.addWidget(QPushButton('guardar seleccion'), 6, 0, 1, 2)
        boton_accion = self.main_box_dock1.itemAtPosition(5, 0).widget()
        boton_accion.clicked.connect(self.realizar_accion)
        self.main_box_dock1.setRowStretch(7,1)
    ...
    def realizar_accion(self):
        print("se preciono realizar accion")
        if self.radio_modificar.isChecked():
            self.crear_pestaña_3D()
        elif self.radio_eliminar.isChecked():
            self.no_permitir()

    def crear_pestaña_3D(self):
        try:
            global flag_creacion_3D
            if flag_creacion_3D:
                #if not self.matplotlib_process.is_alive:  # Verificar si el proceso ya está en marcha
                self.matplotlib_process.start()
                print("matplotlib iniciado")
            elif self.matplotlib_process.is_alive:
                print("El proceso de Matplotlib ya está en marcha")
        except Exception as e:
            print(f"Excepción en iniciar matplotlib: {e}")
    def no_permitir(self):
        global flag_creacion_3D
        flag_creacion_3D = False
        if self.matplotlib_process and self.matplotlib_process.is_alive():
            plt.close()  # Cerrar la ventana de Matplotlib
            self.matplotlib_process.finalizar_process()
