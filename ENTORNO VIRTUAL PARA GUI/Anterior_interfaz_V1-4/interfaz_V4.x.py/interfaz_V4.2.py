...
width_frame_O = 640  # Define el ancho del marco original
height_frame_O = 480  # Define la altura del marco original
plano_cartesiano = np.ones((height_frame_O, width_frame_O, 3), dtype=np.uint8) * 255
    ...
    def run(self):
        ...
        while self.capturando:
            if not self.pausado:
                ...
                global width_frame_O, height_frame_O
                width_frame_O, height_frame_O, _ = frame.shape
                if not ret:
...
class VideoWidget(QWidget):
    ...
    def crear_plano_cartesiano(self):
        global width_frame_O, height_frame_O, plano_cartesiano
        # Dibujar ejes X e Y
        cv2.line(plano_cartesiano, (0, height_frame_O // 2), (width_frame_O, height_frame_O // 2), (0, 0, 0), 2)
        cv2.line(plano_cartesiano, (width_frame_O // 2, 0), (width_frame_O // 2, height_frame_O), (0, 0, 0), 2)

        # Dibujar marcas en los ejes
        for i in range(0, height_frame_O, 50):
            cv2.line(plano_cartesiano, (i, height_frame_O // 2 - 5), (i, height_frame_O // 2 + 5), (0, 0, 0), 2)
        for j in range(0, height_frame_O, 50):
            cv2.line(plano_cartesiano, (height_frame_O // 2 - 5, j), (height_frame_O // 2 + 5, j), (0, 0, 0), 2)
