...
def mostrar_matplotlib_3d(matplotlib_iniciado, cambio_color_queue, parametros_vectores):
    ...
    def actualizar(i):
        try:
            ...
            ax.plot(x_data, y_data, z_data, color=color)
            ...
            # Dibuja la pirámide
            ax.add_collection3d(Poly3DCollection(faces, facecolors=color, linewidths=1, edgecolors='r', alpha=0.5))

            # Define la flecha
            

            # Dibuja la flecha
            ax.quiver(*arrow_start, *arrow_direction, color= color)
