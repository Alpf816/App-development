...
def draw_cube_corners(cube_corners1, title, rvec, tvec):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    cubeCorners2 = np.float32([[0, 0, 0], [0, 6, 0], [6, 6, 0], [6, 0, 0],
                               [0, 0, -6], [0, 6, -6], [6, 6, -6], [6, 0, -6]])

    # Draw the first cube edges
    edges = [[cube_corners1[i], cube_corners1[j]] for i in range(4) for j in range(i+1, 4)]
    edges += [[cube_corners1[i], cube_corners1[i+4]] for i in range(4)]
    edges += [[cube_corners1[i+4], cube_corners1[(i+1)%4+4]] for i in range(4)]

    for edge in edges:
        ax.plot(*zip(*edge), color='r')

    # Draw the second cube edges
    edges2 = [[cubeCorners2[i], cubeCorners2[j]] for i in range(4) for j in range(i+1, 4)]
    edges2 += [[cubeCorners2[i], cubeCorners2[i+4]] for i in range(4)]
    edges2 += [[cubeCorners2[i+4], cubeCorners2[(i+1)%4+4]] for i in range(4)]

    for edge in edges2:
        ax.plot(*zip(*edge), color='b')

    # Plot the rotation vector
    ax.quiver(0, 0, 0, rvec[0], rvec[1], rvec[2], color='g', label='Rotation Vector')

    # Plot the translation vector
    ax.quiver(0, 0, 0, tvec[0], tvec[1], tvec[2], color='m', label='Translation Vector')
...
def poseEstimation(option: DrawOption, video_capture):
    ...
    #cubeCorners = np.float32([[0, 0, -6], [0, 6, -6], [6, 6, -6], [6, 0, -6],[0, 0, 0], [0, 6, 0], [6, 6, 0], [6, 0, 0]])   
    ...
    while True:
        ...
        if cornersFound:
            ...
            if option == DrawOption.CUBE:
                ...
                # Display rotated cube corners
                draw_cube_corners(rotated_cube_corners, "Rotated Cube Corners", rvecs.flatten(), tvecs.flatten())  # Pass tvecs here

                # Draw total side length on the image
                cv.putText(frame, f'Total Side Length: {total_side_length:.2f} units', (20, 50), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv.LINE_AA)
