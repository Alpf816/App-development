...
class ImageProcessorThread(QThread):
    ...
    def __init__(self, get_frame_function, dic_thread,input_queue_ROI1,input_queue_ROI2,input_queue_ROI3,input_queue_ROI4,hilo_video):
        ...
        self.process_1 = VideoProcess(self.O_ROI_C1[0],self.O_ROI_C1[1],self.input_queue_ROI1,self.output_queue_ROI1,self.WH_ROI_C1[0],self.WH_ROI_C1[1],self.TonMin1,self.TonMax1,self.PurMin1,self.PurMax1,self.LumMin1,self.LumMax1,self.KerX1,self.KerY1)
...
##########################################################################aqui se conectan los sliders
            ...
            if frame is not None and frame.size != 0:
                if self.current_preset == "Original":
                    ...
                    if self.process_flag_1:
                        ...
                        while self.process_flag_1 is True and self.process_1.is_alive():
                        #while True:
                            if not self.output_queue_ROI1.empty():
                    ...
                    if self.process_flag_2:
                        try:
                            ...
                            print(f"Excepción en Seleccion de colores --- process 2: {e}")
                    ...
                    if self.process_flag_3:
                        try:
                            ...
                            print(f"Excepción en Seleccion de colores --- process 3: {e}")
                    ...
                    if self.process_flag_4:
                        try:
                            ...
                            print(f"Excepción en Seleccion de colores --- process 4: {e}")
    ...
    def finalizar_process(self):
        ...
        if self.process_1 is not None and self.process_1.is_alive():
            ...
            self.process_1.join()
            if self.process_1 is not None and self.process_1.is_alive():
                print("No se finalizo el process")
            else:
...
class VideoProcess(multiprocessing.Process):
    def __init__(self,O_ROI_H,O_ROI_V,input_queue_ROI,output_queue_ROI,W_ROI,H_ROI,TonMin,TonMax,PurMin,PurMax,LumMin,LumMax,KerX,KerY):
                
        super().__init__()
        self.input_queue = input_queue_ROI
        self.output_queue = output_queue_ROI
        self.O_ROI_H = O_ROI_H
        self.O_ROI_V = O_ROI_V
        self.W_ROI = W_ROI
        self.H_ROI = H_ROI
        self.TonMin = TonMin
        self.TonMax = TonMax
    ...
    def run(self):
        ...
        try:
            ...
            print(f"Excepción en el process 1: {e}")
...
#generacion de UI Principal-----------------------------------------
    def inicializarUI(self):
        ...
        self.setWindowTitle("Interfaz_v6.2") 
