    ...
    def actualizar_cordenadas(self, Valor_process,dic_roi):
        ...
        if Valor_process == 1:
            ...
            if flag_creacion_3D:
                ...
                Dictvect["parametros_vectores"] = str("tipo")
                
                self.parametros_vectores.put(Dictvect)
...
#############################################################################################
class mostrar_matplotlib_3d(multiprocessing.Process):
    ...
    def run(self):
        ...
        def actualizar(i):
            try:
                ...
                ax3d.clear()
                color = self.cambio_color_queue.get() if not self.cambio_color_queue.empty() else last_color
                    ...
                    if "tipo" in dic_parametros:
                        ...
                        if tipo_datos == "parametros_vectores":
                            if "rvecs_1" in dic_parametros and "tvecs_1" in dic_parametros:
                                ...
                                # Aquí puedes realizar operaciones con rvecs y tvecs
                                # Por ejemplo, normalizar los vectores de traslación
                            ...
                            dist1.value = str(distancia_camara_superficie)
                            dist2.value = str(distancia_proyector_superficie)
                            dist3.value = str(distancia_camara_proyector)
...
#Primer dock---------------------------------------------------
    ...
    def create_dock_modificacion_de_datos(self):
        ...
        self.btn_matplotlib = QPushButton("Iniciar Matplotlib")
        self.btn_matplotlib.clicked.connect(lambda: self.iniciar_matplotlib(self.matplotlib_iniciado, self.cambio_color_queue,self.parametros_vectores))

        self.btn_cambiar_color = QPushButton("Cambiar Color")
        self.btn_cambiar_color.clicked.connect(self.cambiar_color)
    ...
    def crear_pestaña_3D(self):
        pass
    def iniciar_matplotlib(self,matplotlib_iniciado, cambio_color_queue,parametros_vectores):
        try:
            
            if not matplotlib_iniciado.value:
                #flag_creacion_3D = True
                matplotlib_process = mostrar_matplotlib_3d(matplotlib_iniciado, cambio_color_queue,parametros_vectores)
